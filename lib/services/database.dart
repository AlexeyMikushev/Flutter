...
class NotesDatabaseService {
  ...
  init() async {
    ...
    path = join(path, 'notesFinal.db');
    print("Entered path $path");

    return await openDatabase(path, version: 1,
        onCreate: (Database db, int version) async {
          await db.execute(
              'CREATE TABLE Notes (_id INTEGER PRIMARY KEY, title TEXT, content TEXT, date TEXT, isNotify INTEGER, dateNotification TEXT);');
  ...
  Future<List<NotesModel>> getNotesFromDB() async {
    ...
    List<Map> maps = await db.query('Notes',
        columns: ['_id', 'title', 'content', 'date', 'isNotify', 'dateNotification']);
    if (maps.length > 0) {
      maps.forEach((map) {
  ...
  Future<NotesModel> addNoteInDB(NotesModel newNote) async {
    ...
    int id = await db.transaction((transaction) {
      transaction.rawInsert(
          'INSERT into Notes(title, content, date, isNotify, dateNotification) VALUES ("${newNote.title}", "${newNote.content}", "${newNote.date.toIso8601String()}", ${newNote.isNotify == true ? 1 : 0} , "${newNote.dateNotification.toIso8601String()}");');
    });
    newNote.id = id;
